// Day 25: Snowverload - Part 1
// https://adventofcode.com/2023/day/25
// Solution to find removed nodes: https://pastebin.com/Chv8CkJs
const fs = require('node:fs');

var snowverload = function (input, removedNodes) {
  ...
  // remove nodes
  for (let i = 0; i < removedNodes.length; i += 2) {
    const a = removedNodes[i];
    const b = removedNodes[i + 1];
    removeEdge(a, b);
  }

  const group1 = count(removedNodes[0]);
  const group2 = count(removedNodes[1]);
  return group1 * group2;

  function getData(input) {
    return input.split('\n').map((line) => line.split(': ').map((m) => m.split(' ').filter(Boolean)));
  }
  ...
  function buildGraph(data) {
    ...
    const graph = {};
    lines.push('graph {');
    for (const [[source], targets] of data) {
      if (!graph[source]) graph[source] = new Set();
      for (const target of targets) {
        if (!graph[target]) graph[target] = new Set();
    ...
    lines.push('}');
    //fs.writeFileSync('./graph.dot', lines.join('\n'));
    return graph;
  }
...
rzs: qnr cmg lsr rsh
frs: qnr lhk lsr`;
var removedNodes = ['hfx', 'pzl', 'bvb', 'cmg', 'nvd', 'jqt'];
var expected = 54;
var result = snowverload(input, removedNodes);
console.log(result, result === expected);

var input = `bdq: hfr lnm
rfq: jph lqj zrv
...
dfq: dxr jgc mjs mjd
zgj: khj`;
var removedNodes = ['jhq', 'zkt', 'pgt', 'lnr', 'vph', 'tjz'];
var expected = 547410;
var result = snowverload(input, removedNodes);
console.log(result, result === expected);

console.timeEnd('day-25_part-1');
