...
var aplenty = function (input) {
  let totalPartRating = 0;
  const [workflowMap, partRatings] = getData(input);
  for (const partRatingMap of partRatings) {
    let workflowName = 'in';
    while (workflowMap.has(workflowName)) {
      let found = false;
      const { conditions, otherWorkflow } = workflowMap.get(workflowName);
      for (const { part, op, value, nextWorkflow } of conditions) {
        if (!partRatingMap.has(part)) continue;
        const rating = partRatingMap.get(part);
        if ((op === '>' && rating > value) || (op === '<' && rating < value)) {
    ...
    if (workflowName === 'A') {
      const rating = [...partRatingMap.values()].reduce((sum, num) => sum + num, 0);
      console.log(rating);
      totalPartRating += rating;
    }
  }
  return totalPartRating;
