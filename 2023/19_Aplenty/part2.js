...
var aplenty = function (input) {
  const [MIN, MAX] = [1, 4000];
  const [workflowMap] = getData(input);
  ...
  for (const result of results) {
    ...
    for (const [part, start, end] of result) {
      let [min, max] = map.get(part);
      min = Math.max(min, start);
      max = Math.min(max, end);
      map.set(part, [min, max]);
    }

    let count = 1;
    for (const [, [min, max]] of map) {
      count *= max - min + 1;
    }
    total += count;
  }
  ...
  function getData(input) {
    const [workflowInput, ratingInput] = input.split('\n\n').map((block) => block.split('\n'));
    const workflowMap = new Map();
    workflowInput.forEach((line) => {
      const [name, conditionInput] = line.split(/[{}]/g);
      const conditionItems = conditionInput.split(',');
      const otherWorkflow = conditionItems.pop();
      const conditions = conditionItems.map((condition) => {
        const found = condition.match(/(?<op><|>|:)/g);
        const [part, value, nextWorkflow] = condition.split(/[<>:]/g);
        return { part, op: found[0], value: Number(value), nextWorkflow };
      });
      workflowMap.set(name, { conditions, otherWorkflow });
    });

    const partRatings = ratingInput.map((line) => {
      const items = line.slice(1, line.length - 1).split(',');
      const map = new Map();
      for (const item of items) {
        const [part, value] = item.split('=');
        map.set(part, Number(value));
      }
      return map;
    });
    return [workflowMap, partRatings];
  }

  function getValues(workflowName, queue) {
    ...
    const { conditions, otherWorkflow } = workflowMap.get(workflowName);
    for (let i = 0; i < conditions.length; i++) {
      const { part, op, value, nextWorkflow } = conditions[i];
      if (op === '<') {
        getValues(nextWorkflow, [...queue, [part, MIN, value - 1]]);

        if (!conditions[i + 1]) {
          getValues(otherWorkflow, [...queue, [part, value, MAX]]);
        } else {
          queue.push([part, value, MAX]);
        }
      } else {
        getValues(nextWorkflow, [...queue, [part, value + 1, MAX]]);

        if (!conditions[i + 1]) {
          getValues(otherWorkflow, [...queue, [part, MIN, value]]);
        } else {
          queue.push([part, MIN, value]);
        }
      }
    }
  }
...
var result = aplenty(input);
console.log(result, result === expected);
console.log(expected);

var input = `lx{a>1404:kq,a>736:hmc,hbb}
...
var expected = 121464316215623;
var result = aplenty(input);
console.log(result, result === expected);
console.log(expected);

console.timeEnd('day-19_part-2');
